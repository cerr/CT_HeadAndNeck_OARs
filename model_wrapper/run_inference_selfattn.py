...
def main(input_path, session_path, output_path, DCMexportFlag=False):
    ...
    if out_type=='DCM':
        # Export to DICOM
        struct_file_path = mask_to_DICOM(pt_id, model_arch, output_path, proc_str_num, orig_scan_num, planC)
    elif out_type=='NII':
        print("****** Export structure to NIfTI ******")
        label_out_file_name = f"{pt_id}_{model_arch}_AI_seg.nii.gz"
        struct_file_path = os.path.join(output_path, label_out_file_name)
        write_file(proc_label_map, struct_file_path, orig_img)

    return struct_file_path, proc_str_num, planC
