...
def export_seg(proc_str_list, output_dcm_path, planC):
    """Export list of structures to DICOM"""
    series_description = "AI Generated"
    export_opts = {'seriesDescription': seriesDescription}
    rtstruct_iod.create(proc_str_list, output_dcm_path, planC, export_opts)

    return 0


def main(input_path, session_path, output_path, DCMexportFlag=False):
    ...
    print("****** Export structure to DICOM ******")
    mask_out_file_name = f"{pt_id}_{model_arch}_AI_seg.dcm"
    mask_out_file_path = os.path.join(output_path, label_out_file_name)
    export_seg(proc_str_num, mask_out_file_path, planC)


    return mask_out_file_path, proc_str_num, planC

if __name__ == "__main__":
    opt = TrainOptions().parse()
    main(opt, sys.argv[1], sys.argv[2], sys.argv[3])
